...
object `conditions and pattern-matching`:
  ...
  object `if-then-else`:
    ...
    object `if-then-else-expression`:
      ...
      /**
       * if-then-else is an expression that returns a value. It is not an operator that "does something"; it produces a result.
       */
      val age = 18
  ...
  object `pattern-matching`:
    ...
    object `pattern-matching-basics`:
      ...
      /**
       * Pattern matching is an expression that selects the first matching case. Each case returns a value, so the whole match returns a value too.
       */
      val dayNumber = 6
    ...
    object `list-decomposition`:
      ...
      /**
       * Lists can be decomposed with patterns like head :: tail. This lets us work with the first elements and the rest of the list.
       */
    ...
    object `sealed-trait-exhaustiveness`:
      ...
      /**
       * Sealed traits define a closed hierarchy of subtypes. The compiler knows all possible cases and can check exhaustiveness.
       */
      sealed trait Shape
      ...
      /**
       * If we forget a case, the compiler warns: "Match is not exhaustive". That warning is useful because it points to the missing cases.
       *
